...
class Run:
    def __init__(self, factory):
        ...
        self.controller = PIDController(320, 10, 60, [-10, 10], [-200, 200], is_angle=True) # initialize PIDcontroller for go to goal
        self.controller_s = PIDController(1000, 0, 50, [0, 0], [-200, 200], is_angle=False) # initialize PIDcontroller for go to goal
        ...
        self.waypoints = [
                            ...
                            # [0.0, 1.5, 0.9],
                            # [0.0, 0.0, 0.5]
                            [8.0 * 0.305, 6.0 * 0.305, 0.5],
                            [7.0 * 0.305, 1.0 * 0.305, 0.5],
                            [1.0 * 0.305, 6.0 * 0.305, 0.5],
                            [0.0 * 0.305, 0.0 * 0.305, 0.5],
                        ]
        self.index = 0
    ...
    def followingWall(self):
        ...
        while(True):
            ...
            #print(current)
            self.goal = 0.3 / math.sin(math.radians(math.fabs(self.angle)))
    ...
    def goToGoal(self):
        self.controller.init(320, 10, 60, [-10, 10], [-200, 200], is_angle=True) # initialize PIDcontroller for go to goal
        self.controller_s.init(1000, 0, 50, [0, 0], [-200, 200], is_angle=False) # initialize PIDcontroller for go to goal
        ...
        while(True):         
            ...
            if state is not None:
                ...
                output_distance = max(output_distance, 150)
