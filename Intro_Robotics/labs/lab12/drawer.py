...
class Drawer:
    ...
    def go_to_goal_curve(self, points, draw = False, color=(0.0, 1.0, 0.0)):
        self.pidTheta.init(300, 5, 50, [-10, 10], [-200, 200], is_angle=True)
    ...
    def go_to_theta(self, goal_theta):        
        ...
        while True:
            ...
            #print("go to angle [%f], [%f]" % (current_theta, goal_theta))

            # if current_theta > math.pi:
            #     current_theta = -1.0 * (math.pi * 2 - current_theta)
            # elif current_theta < (-1 * math.pi):
            #     current_theta = math.pi * 2 + current_theta
            error = math.atan2(math.sin(goal_theta - current_theta), math.cos(goal_theta - current_theta))
            if math.fabs(error) < self.theta_threshold:
                self.create.drive_direct(0, 0)
                break
